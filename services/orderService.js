...
const orderService = {
    ...
    return Cart.findByPk(req.body.cartId, {
      ...
      return Order.create({
        ...
        .then(order => {
          ...
          callback({ status: 'success', message: "", order, PaymentTypeId, ShipmentTypeId, discount });
        })
    })
  },

  getOrderSuccess: (req, res, callback) => {
    Order.findByPk(req.params.id, {
      ...
      return callback({ order, originAmount, shippingFee, subtotal })
    })
  },
  ...
  getBranchSelection: (req, res, callback) => {
    Order.findByPk(req.params.id, {
      include: [User, { model: Product, as: "items", include: [CartItem] }]
    }).then(order => {
      ...
      order
        ...
        .then(order => {
          callback({ order, pickupInfo });
        });
    });
  },

  pickupCallback: (req, res, callback) => {
